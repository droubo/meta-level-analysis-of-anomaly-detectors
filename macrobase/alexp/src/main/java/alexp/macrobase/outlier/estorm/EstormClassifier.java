...
public class EstormClassifier extends MultiMetricClassifier implements ParametersAutoTuner {
    ...
    private ExactStorm estorm;

    private DataFrame output;

    public EstormClassifier(String columnName) {
        super(columnName);
    }

    public EstormClassifier(String[] columns) {
        super(columns);
    }
    ...
    public void process(DataFrame input) throws Exception {
        ...
        if (estorm == null) { // init (can't do it in constructor because of the setters. Maybe should make them constructor parameters, but others classifiers don't do it that way)
            estorm = new ExactStorm(maxDistance, minNeighborCount, windowSize, slide);
        }
        ...
        ArrayList<Data> estormData = new ArrayList<>();
        double[][] metricRows = new double[input.getNumRows()][columns.length];
        for (int i = 0; i < input.getNumRows(); i++) {
            for (int j = 0; j < columns.length; j++) {
                metricRows[i][j] = metricColumns.get(j)[i];
            }
            estormData.add(new Data((int) timeColumn[i], metricRows[i]));
            timeIndexMap.put((int) timeColumn[i], i);
        }
        ArrayList<Data> outliers = estorm.detectOutlier(estormData, (int) timeColumn[timeColumn.length - 1]);

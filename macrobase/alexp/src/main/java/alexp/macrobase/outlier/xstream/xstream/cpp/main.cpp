...
int main(int argc, char *argv[]) {
  ...
  chrono::time_point<chrono::steady_clock> start_train;
  chrono::time_point<chrono::steady_clock> end_train;
  chrono::time_point<chrono::steady_clock> start_score;
  chrono::time_point<chrono::steady_clock> end_score;
  chrono::milliseconds diff;
  chrono::nanoseconds diff_nano;
  chrono::milliseconds diff_training;
  vector<chrono::nanoseconds::rep> diff_train;
  vector <chrono::milliseconds::rep> diff_score;
  chrono::microseconds::rep sum_of_elems;
  ...
  //string filename = args["--input"].asString();
...
*/
  ...
  anomalyscores.reserve(10000000);
  ...
  while (getline(cin, line)) {
    ...
    if (row_idx == init_sample_size) {
	  ...
	  diff_training = chrono::duration_cast<chrono::milliseconds>(end - start);
    ...
    if (nwindows <= 0) { // non-windowed mode
	  ...
	  start_train = chrono::steady_clock::now();
      window[window_size] = xp;
      window_size++;
	  ...
	  end_train = chrono::steady_clock::now();
	  diff_nano = chrono::duration_cast<chrono::nanoseconds>(end_train - start_train);
	  diff_train.push_back(diff.count());
	...
	/*
	start_score = chrono::steady_clock::now();
    if ((row_idx > init_sample_size) && (row_idx % scoring_batch_size == 0)) {
	  ...
	  end_score = chrono::steady_clock::now();
	  diff = chrono::duration_cast<chrono::milliseconds>(end_score - start_score);
	  diff_score.push_back(diff.count());
      //cout << endl;
    }
	*/
    row_idx++;
  ...
  diff = chrono::duration_cast<chrono::milliseconds>(end - start);
  cerr << "done in " << diff.count() << "ms" << endl;
  cerr << "TRAINED IN " << diff_training.count() << endl;
  sum_of_elems = std::accumulate(diff_score.begin(), diff_score.end(),
	  decltype(diff_score)::value_type(0));
  cerr << "SCORED IN " << sum_of_elems/diff_score.size() << endl;
  sum_of_elems = std::accumulate(diff_train.begin(), diff_train.end(),
	  decltype(diff_train)::value_type(0));
  cerr << "UPDATED IN " << sum_of_elems / diff_train.size() << endl;
  cout.rdbuf(coutbuf);
}

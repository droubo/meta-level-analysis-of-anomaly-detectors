...
public class RCFClassifier extends MultiMetricClassifier implements Trainable, Updatable {
    ...
    public void train(DataFrame input) { // RCF model must be initialize only once
        if (trainable) {
            ...
            if (shingleMode()) {
                ...
                inputWindow = DataFrameUtils.toRowArray(removeOutliers(input), columns); // Option 1: Remove outliers
                //inputWindow = DataFrameUtils.toRowArray(input, columns);               // Option 2: Allow outliers
            }
    ...
    public void process(DataFrame input) {
        if (!trainable) {
            if (processable) {
                System.out.println("Processing W" + processWindowCounter);
    ...
    private void outputBuilder(DataFrame windowDF, double[] windowScores) {
        ...
        double[] synSubspaces = new double[windowScores.length];

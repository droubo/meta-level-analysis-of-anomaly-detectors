...
public class McodClassifier extends MultiMetricClassifier implements ParametersAutoTuner {
    ...
    public void process(DataFrame input) throws Exception {
        ...
        //for (Data p : outliers) {
            //System.out.println(p.criterion);
            //System.out.println(p.minDistance);            
        //}
        for (Data p : outliers) {
            final Integer ind = timeIndexMap.get(p.arrivalTime());
            if (ind != null) {
                // System.out.println("OUTLIER CRITERION (REASON THAT HAS BEEN OUTLIER) = "+outlier.criterion);
                resultColumn[ind] =  p.criterion; // reverse the scores to be valid
    ...
    public Map<String, Object> tuneParameters(DataFrame trainSet) {
        ...
        double random_r = ThreadLocalRandom.current().nextDouble(0.1, 6); // default range was 0.1 up to 6.

...
while True:
    ...
    candidates = []
    kminusone = [s for (s, c) in frequent_item_sc]
    kminusone_set = set(kminusone)

    # grow
    for i in range(0, len(kminusone)):
        for j in range(i+1, len(kminusone)):
            print kminusone[i], kminusone[j]
            candidateset = set(kminusone[i]) | set(kminusone[j])

            has_support = True
            for subset in combinations(candidateset, len(candidateset)-1):
                if subset not in kminusone_set:
                    has_support = False
                    break
            if has_support:
                candidates.append(candidateset)
